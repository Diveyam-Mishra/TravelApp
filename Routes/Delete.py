...
async def deleteAvatar(current_user: User=Depends(get_current_user), db: AsyncSessionLocal=Depends(get_db), blob_client=Depends(get_blob_service_client)):
    if current_user is None:
        raise HTTPException(status_code=401, detail="Unauthorized")
...
async def deleteCarousel(fileName:str, current_user: User=Depends(get_current_user), db: AsyncSessionLocal=Depends(get_db), blob_service_client=Depends(get_blob_service_client)):
